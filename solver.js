export class SolverManager {
    ...
    // Get facelets from scramble string
    fromScramble(scramble) {
        if (typeof min2phase === 'undefined' || !min2phase.fromScramble) {
            console.error("min2phase.fromScramble not available");
            return null;
        }
        // min2phase.fromScramble(s) returns facelets string
        return min2phase.fromScramble(scramble);
    }
    ...
    solve(cubeState) {
        ...
        // min2phase does not support explicit verify in this version.
        // We rely on solve() to handle it or return error.
